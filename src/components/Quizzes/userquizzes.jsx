...
const UserQuizzes = () => {
  ...
  const user = useSelector(state => state.auth); // Assuming your user is stored in `state.user`
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      try {
        // Fetch quizzes created by the user based on their uid
        const q = query(collection(db, 'quizzes'), where('createdBy', '==', user.uid));  // Query for user's quizzes
    ...
    fetchQuizzes();
  }, [user]);  // Fetch quizzes only when the user is available
