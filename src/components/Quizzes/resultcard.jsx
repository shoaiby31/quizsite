...
import {
    Box, Card, CardContent, Typography, Divider, Chip, useTheme, Button, Stack, Snackbar,
    Alert, Paper, Grid
} from '@mui/material';
...
const COLORS = ["#47bf99", "#f04135"]; // Green for correct, Pink for incorrect
...
function ResultCard({ data, score, length, totalScore, quizId, title, attemptId }) {
    ...
    const handleRetake = async () => {
        try {
            ...
            // Delete attempts where userId and quizId match
            const attemptsRef = collection(db, "attempts");
            const q = query(attemptsRef, where("quizId", "==", quizId), where("userId", "==", userId));
            const querySnapshot = await getDocs(q); // ✅ use getDocs, not getDoc
            ...
            <Card sx={{ borderRadius: 4, width: '100%', boxShadow: '0 12px 32px rgba(0,0,0,0.15)', }}>
                <CardContent sx={{ padding: 5 }}>
                    <Grid container>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                            <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="right" spacing={2} mt={5}>
                    ...
                    <Grid container>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <Box textAlign="center" mt={2}>
                                ...
                                <Typography variant="body1" mt={1.5} color="text.secondary">You got {((score / total) * 100).toFixed(1)}% of the questions correct.</Typography>
                            
                            </Box>
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <Box component={motion.div} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} sx={{
                                width: '100%', height: 360, p:5, position: 'relative', backgroundColor: theme.palette.background.paper, borderRadius: 4
                            }}>
